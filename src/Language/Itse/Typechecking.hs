...
freeNameTms (Kind _k) = case _k of
  Kind_Unit -> []
  Kind_PiTm x t k -> (freeNameTms (Type t) ++ freeNameTms (Kind k)) `withoutName` x
  Kind_PiTy _ k l -> freeNameTms (Kind k) ++ freeNameTms (Kind l)
...
freeNameTys (Kind _k) = case _k of
  Kind_Unit -> []
  Kind_PiTm _ t k -> freeNameTys (Type t) ++ freeNameTys (Kind k)
  Kind_PiTy x k l -> (freeNameTys (Kind k) ++ freeNameTys (Kind l)) `withoutName` x
...
synthesizeKind ctx _t = case _t of
  ...
  Type_AbsTm x s t -> do
    ...
    checkKind ctx t Kind_Unit
    -- ctx |- λ x : s . t :: Π x : s . k
    return $ Kind_PiTm x s k
  -- s t
  Type_AppTy s t -> do
    -- ctx |- s :: Π x : k . l
    (x, k, l) <-
      synthesizeKind ctx s >>= \case
        Kind_PiTy x k l -> return (x, k, l)
...
synthesizeNameTy :: Context -> Name Term -> M Type
synthesizeNameTy Context_Empty x =
  throwError $ printf "could not synthesize the type of term name: `%s`" (show x)
synthesizeNameTy (Context_Typing ctx y t) x =
  if x == y
    then return t
    else synthesizeNameTy ctx x
synthesizeNameTy (Context_Kinding ctx _ _) x =
  synthesizeNameTy ctx x
synthesizeNameTy (Context_Closure ctx clo) x =
  undefined

synthesizeNameKd :: Context -> Name Type -> M Kind
synthesizeNameKd Context_Empty x =
  throwError $ printf "could not synthesize the kind of type name: `%s`" (show x)
synthesizeNameKd (Context_Typing ctx _ _) x =
  synthesizeNameKd ctx x
synthesizeNameKd (Context_Kinding ctx y k) x =
  if x == y
    then return k
    else synthesizeNameKd ctx x
synthesizeNameKd (Context_Closure ctx clo) x =
  undefined -- TODO
...
unify e1 e2 =
  ...
  where
    ...
    go (Kind _k1) (Kind _k2) = case (_k1, _k2) of
      (Kind_Unit, Kind_Unit) ->
        return ()
      (Kind_PiTm x1 t1 k1, Kind_PiTm x2 t2 k2) -> do
        go (Type t1) $ substitute x2 (Term $ Term_Ref x1) (Type t2)
        go (Kind k1) $ substitute x2 (Term $ Term_Ref x1) (Kind k2)
      (Kind_PiTy x1 k1 l1, Kind_PiTy x2 k2 l2) -> do
        go (Kind k1) $ substitute x2 (Type $ Type_Ref x1) (Kind k2)
